...
class Option(abc.ABC):
  ...
  def calcOptionPriceDiff(self) -> decimal.Decimal:
    """Calculate the difference in price of the put/call when the trade was placed versus its current value.
      Specifically, diff = original price - current price.
  ...
  def updateOption(self, updatedOption: 'Option') -> None:
    ...
    if self.underlyingTicker in updatedOption.underlyingTicker and self.strikePrice == updatedOption.strikePrice and (
      ...
      logging.info('originalTicker: {} tickerToUpdate: {} originalStrike: {} strikeToUpdate: {} originalExp: {} expToUpdate: {} '.format(
        self.underlyingTicker, updatedOption.underlyingTicker, self.strikePrice, updatedOption.strikePrice,
        self.expirationDateTime, updatedOption.expirationDateTime))
