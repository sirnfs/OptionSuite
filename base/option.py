...
class Option(abc.ABC):
    """This class defines the basic type for the backtester -- an option.
    Put and call are derived from this class.
    ...
    Attributes:
      ...
      optionSymbol:  special market symbol used to denote option.
      bidPrice:  current bid price of option.
      askPrice:  current asking price of option.
      tradePrice:  price of the option when trade was executed / put on.
      settlementPrice: settlement price of the option; could be the mean of bid / ask but not always.
      openInterest:  number of open option contracts.
      volume:  number of contracts traded.
      dateTime:  date / time of last updated option chain (this comes from the CSV).
      tradeDateTime: date / time that option was put on (this is populated once the option is added to the portfolio).
    ...
    def calcOptionPriceDiff(self) -> decimal.Decimal:
        """Calculate the difference in price of the put/call when the trade was put on versus its current value.

           :return: price difference (original price - current price).
        """
        return self.tradePrice - self.settlementPrice
    ...
    def updateOption(self, updatedOption: 'Option') -> None:
        ...
        if self.underlyingTicker in updatedOption.underlyingTicker and (
               self.strikePrice == updatedOption.strikePrice) and (
            ...
            self.settlementPrice = updatedOption.settlementPrice
            ...
            logging.info(
                'originalTicker: {} tickerToUpdate: {} originalStrike: {} strikeToUpdate: {} originalExp: {}'
                ' expToUpdate: {} '.format(self.underlyingTicker, updatedOption.underlyingTicker,
                                           self.strikePrice, updatedOption.strikePrice, self.expirationDateTime,
                                           updatedOption.expirationDateTime))
