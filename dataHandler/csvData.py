...
class CsvData(dataHandler.DataHandler):
  ...
  def __openDataSource(self) -> Mapping[Text, int]:
    ...
    try:
      with open('/Users/msantoro/PycharmProjects/Backtester/dataHandler/dataProviders.json') as dataProvider:
  ...
  def __createBaseType(self, optionChain: pd.DataFrame) -> Iterable[option.Option]:
    ...
    optionFieldDict = {'underlyingTicker': None, 'strikePrice': None, 'delta': None, 'expirationDateTime': None,
                       ...
                       'settlementPrice': None, 'tradePrice': None, 'openInterest': None, 'volume': None,
                       'dateTime': None, 'theta': None, 'gamma': None, 'rho': None, 'vega': None, 'impliedVol': None,
                       'exchangeCode': None, 'exercisePrice': None, 'assignPrice': None, 'openCost': None,
                       'closeCost': None,
                      }
    ...
    for _, row in optionChain.iterrows():
      ...
      # index options. Let's check if settlmentPrice is None or empty.
      if optionFieldDict['settlementPrice'] is None or not optionFieldDict['settlementPrice']:  # For index options
        ...
        optionFieldDict['settlementPrice'] = optionFieldDict['tradePrice']
      else:  # For future options.
        optionFieldDict['tradePrice'] = optionFieldDict['settlementPrice']

      argsDict = {'underlyingTicker': optionFieldDict['underlyingTicker'] if optionFieldDict[
                    'underlyingTicker'] else None,
                  'strikePrice': decimal.Decimal(optionFieldDict['strikePrice']) if optionFieldDict[
                    'strikePrice'] else None,
                  'delta': float(optionFieldDict['delta']) if optionFieldDict['delta'] else None,
                  'expirationDateTime': datetime.datetime.strptime(
                    optionFieldDict['expirationDateTime'], dataProviderConfig['date_time_format']) if optionFieldDict[
                    'expirationDateTime'] else None,
                  'underlyingPrice': decimal.Decimal(optionFieldDict['underlyingPrice'] if optionFieldDict[
                    'underlyingPrice'] else None),
                  'optionSymbol': optionFieldDict['optionSymbol'] if optionFieldDict['optionSymbol'] else None,
                  'bidPrice': decimal.Decimal(optionFieldDict['bidPrice']) if optionFieldDict['bidPrice'] else None,
                  'askPrice': decimal.Decimal(optionFieldDict['askPrice']) if optionFieldDict['askPrice'] else None,
                  'settlementPrice': decimal.Decimal(optionFieldDict['settlementPrice']) if optionFieldDict[
                    'settlementPrice'] else None,
                  'tradePrice': decimal.Decimal(optionFieldDict['tradePrice']) if optionFieldDict[
                    'tradePrice'] else None,
                  'openInterest': int(optionFieldDict['openInterest']) if optionFieldDict['openInterest'] else None,
                  'volume': int(optionFieldDict['volume']) if optionFieldDict['volume'] else None,
                  'dateTime': datetime.datetime.strptime(optionFieldDict['dateTime'],
                                                         dataProviderConfig['date_time_format']) if optionFieldDict[
                    'dateTime'] else None,
                  'tradeDateTime': datetime.datetime.strptime(
                    optionFieldDict['dateTime'], dataProviderConfig['date_time_format']) if optionFieldDict[
                    'dateTime'] else None,
                  'theta': float(optionFieldDict['theta']) if optionFieldDict['theta'] else None,
                  'gamma': float(optionFieldDict['gamma']) if optionFieldDict['gamma'] else None,
                  'rho': float(optionFieldDict['rho']) if optionFieldDict['rho'] else None,
                  'vega': float(optionFieldDict['vega']) if optionFieldDict['vega'] else None,
                  'impliedVol': float(optionFieldDict['impliedVol']) if optionFieldDict['impliedVol'] else None,
                  'exchangeCode': optionFieldDict['exchangeCode'] if optionFieldDict['exchangeCode'] else None,
                  'exercisePrice': decimal.Decimal(optionFieldDict['exercisePrice']) if optionFieldDict[
                    'exercisePrice'] else None,
