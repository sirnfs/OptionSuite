...
class NakedPut(optionPrimitive.OptionPrimitive):
  """This class sets up the naked put option primitive.
    ...
    Attributes:
      orderQuantity:  number of put naked puts.
      ...
      buyOrSell:  Indicates if we want to the put to be long (buy) or short (sell).
  """
  ...
  def getTradeDateTime(self) -> Optional[datetime.datetime]:
    """Gets the date/time for when the option was added to the naked put."""
  ...
  def calcProfitLoss(self) -> decimal.Decimal:
    """Calculate the profit and loss for the naked put using option values when the trade
  ...
  def getBuyingPower(self) -> decimal.Decimal:
    ...
    buying power short put -- greatest of the following * number of contracts * 100:
    ...
    if self.__buyOrSell == optionPrimitive.TransactionType.BUY:
      ...
      buyingPower1 = decimal.Decimal(-0.8)*self.__putToBuyOrSell.underlyingPrice + self.__putToBuyOrSell.strikePrice + (
        currentPutPrice)
      buyingPower2 = decimal.Decimal(0.1)*self.__putToBuyOrSell.strikePrice + currentPutPrice
      buyingPower3 = decimal.Decimal(2.50)
      maxBuyingPower = max(buyingPower1, buyingPower2, buyingPower3)
  ...
  def getCommissionsAndFees(self, openOrClose: Text, pricingSource: Text, pricingSourceConfig: Dict[Any, Any]) -> \
    ...
    if pricingSource == 'tastyworks':
      ...
      if openOrClose == 'open':
        ...
        raise TypeError('Only open or close types can be provided to getCommissionsAndFees().')
      return decimal.Decimal(esOptionConfig['commission_per_contract'] + esOptionConfig['clearing_fee_per_contract'] +
  ...
  def updateValues(self, tickData: Iterable[option.Option]) -> bool:
    ...
    if matchingPutToBuyOrSellOption.settlementPrice is None:
      logging.warning("Settlement price was zero for the put to update; won't update. See below")
