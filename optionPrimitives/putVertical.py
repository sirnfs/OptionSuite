...
class PutVertical(optionPrimitive.OptionPrimitive):
  ...
  def getTradeDateTime(self) -> Optional[datetime.datetime]:
    """Gets the date/time for when the option was added to the vertical."""
    if self.__putToBuy.tradeDateTime is not None:
      return self.__putToBuy.tradeDateTime
    return None
  ...
  def getBuyingPower(self) -> decimal.Decimal:
    ...
    if self.__buyOrSell == optionPrimitive.TransactionType.BUY:
      buyingPower = (currentPutToBuyPrice - currentPutToSellPrice) * self.__numContracts * self.__contractMultiplier
      if buyingPower <= 0:
  ...
  def getCommissionsAndFees(self, openOrClose: Text, pricingSource: Text, pricingSourceConfig: Dict[Any, Any]) -> \
    ...
    """
    # TODO(msantoro): Instead of checking the pricingSource, we should make it such that we can iterate through
    # all fields for a given pricing source to calculate the commissions and fees.
  ...
  def updateValues(self, tickData: Iterable[option.Option]) -> bool:
    ...
    for currentOption in tickData:
      if (putTicker in currentOption.underlyingTicker and currentOption.strikePrice == putStrike and (
        currentOption.expirationDateTime == putExpiration) and currentOption.optionType == option.OptionTypes.PUT):
    ...
    # This check if here because of data errors in the CSV.
    if totCreditDebit <= 0:
      logging.warning('Check bid/ask spread in CSV data for errors.')
    ...
    # This is a CSV data error case. Return False if the %profit is > 100 so that option is not updated, and the option
    # will be removed in portfolio.py.
    if percentProfitLoss > 100:
