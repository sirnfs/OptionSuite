...
class TestPutVerticalOnDownMoveStrategy(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.curStrategy = putVerticalOnDownMoveStrat.PutVerticalOnDownMoveStrat(
      self.signalEventQueue, self.percentDownToTrigger, self.numberDaysForMovingAverage, self.optPutToBuyDelta,
      self.maxPutToBuyDelta, self.minPutToBuyDelta, self.optPutToSellDelta, self.maxPutToSellDelta,
      self.minPutToSellDelta, self.underlyingTicker, self.orderQuantity, self.contractMultiplier, self.riskManagement,
      self.pricingSource, self.pricingSourceConfigFile, self.expCycle, self.optimalDTE, self.minimumDTE,
      self.maximumDTE, maxBidAsk=self.maxBidAsk, maxCapitalToUsePerTrade=self.maxCapitalToUsePerTrade)
    self.portfolioNetLiquidity = decimal.Decimal(100000)
    self.availableBuyingPower = decimal.Decimal(50000)

  # TODO(msantoro): Disabled test since we disabled expiration cycles to support other than monthly.
  ...
  #                                                                       self.minPutToBuyDelta, self.optPutToSellDelta,
  #                                                                       self.maxPutToSellDelta, self.minPutToSellDelta,
  #                                                                       self.underlyingTicker, self.orderQuantity,
  #                                                                       self.riskManager, expCycle, self.optimalDTE,
  ...
  def testUpdateWithOptimalOptionDTEGreaterThanMaximum(self):
    """Tests that no signal event is created if the number of days to expiration is greater than maximum."""
    callOption = self.optionChain.getData()[0]
    putOption = self.optionChain.getData()[1]
    # Modify expiration to be a monthly expiration, but set the number of days such that it is greater than
    # self.maximumDTE.
    callOption.expirationDateTime = datetime.fromisoformat('2014-08-15')
    putOption.expirationDateTime = datetime.fromisoformat('2014-11-15')
    testOptionChain = [callOption, putOption]
    event = tickEvent.TickEvent()
    event.createEvent(testOptionChain)
    self.curStrategy.checkForSignal(event, self.portfolioNetLiquidity, self.availableBuyingPower)
