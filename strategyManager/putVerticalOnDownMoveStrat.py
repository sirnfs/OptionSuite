...
"""
  ...
  def __init__(self, eventQueue:queue.Queue, percentDownToTrigger: float, numberDaysForMovingAverage: int,
               ...
               contractMultiplier: int, riskManagement: riskManagement.RiskManagement, pricingSource: Text,
               pricingSourceConfigFile: Text, expCycle: Optional[strategy.ExpirationTypes]=None,
               optimalDTE: Optional[int]=None, minimumDTE: Optional[int]=None, maximumDTE: Optional[int]=None,
               minimumROC: Optional[float]=None, minCredit: Optional[decimal.Decimal]=None,
  ...
  def __updateWithOptimalOption(self, currentOption: option.Option,
    ...
    # if currentOption.underlyingTicker != self.underlyingTicker:
  ...
  def checkForSignal(self, event: tickEvent, portfolioNetLiquidity: decimal.Decimal,
    ...
    if (optimalPutOptionToBuy and optimalPutOptionToSell) and (
      ...
      opening_fees = putVerticalObj.getCommissionsAndFees('open', self.pricingSource, self.pricingSourceConfig)
